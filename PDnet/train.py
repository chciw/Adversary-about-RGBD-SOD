...
# exit()
model_checkpoint = ModelCheckpoint('checkpoints/vgg161_224x224_weight.{val_loss:.3f}.hdf5', monitor='val_loss',
                                   verbose=1, save_weights_only=True, period=15, save_best_only=True)

train_continue = 1
if train_continue:
    model.load_weights('vgg161_merge_224x224x2_weight.0.168.hdf5', by_name=False)

mode_lr = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.7, patience=1, verbose=1, mode='auto',
...
model.fit(images, masks, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(val_x, val_y),
          callbacks=[model_checkpoint, mode_lr])