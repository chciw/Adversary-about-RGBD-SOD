...
#     get placeholder and output of graph
#     graph_op = tf.get_default_graph().get_operations()
#     with open('output.txt', 'w') as f:
#       for i in graph_op:
#         f.write(str(i))

    nat_dict = {'input_1:0':x_batch,'input_2:0':d_batch,'conv2d_9_target:0':y_batch,
                'conv2d_9_sample_weights:0':np.ones(y_batch.shape[0])}
    adv_dict = {'input_1:0':x_batch_adv,'input_2:0':d_batch_adv,'conv2d_9_target:0':y_batch,
                'conv2d_9_sample_weights:0':np.ones(y_batch.shape[0])}
